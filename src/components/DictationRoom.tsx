...
export default function DictationRoom({
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex flex-col items-center justify-center p-6 bg-page text-text-body">
      
      {/* Nút quay lại */}
      <button
        onClick={onBack}
        className="self-start mb-6 px-4 py-2 rounded-[var(--radius-card)] bg-card border border-[rgba(0,0,0,0.1)] hover:text-primary hover:border-primary-dark hover:bg-primary-light transition-all text-xs font-bold flex items-center gap-1.5 text-text-body shadow-sm"
      >
        <ArrowLeft className="w-4 h-4" /> Về Bảng Điều Khiển
      </button>

      {/* Thẻ luyện nghe trung tâm */}
      <div className="w-full max-w-2xl rounded-[var(--radius-card)] border border-[rgba(0,0,0,0.1)] bg-card p-8 space-y-8 shadow-2xl relative overflow-hidden">
        {/* Glow */}
        <div className="absolute top-0 right-0 w-80 h-80 bg-primary/5 rounded-full blur-[80px] pointer-events-none" />

        {/* Room Header */}
        <div className="flex items-center justify-between border-b border-[rgba(0,0,0,0.1)] pb-4 z-10 relative">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-[var(--radius-card)] bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400 shadow">
          ...
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full text-[9px] font-extrabold bg-primary text-white border-[var(--c-border)] border-primary-dark shadow-[0_4px_0_var(--c-primary-dark)]/10 text-indigo-400 border border-indigo-500/20">
              Focus Mode
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-6 space-y-3 z-10 relative">
          ...
          <span className="text-[10px] font-black uppercase text-text-muted tracking-widest font-mono">
            {isPlaying ? "Đang đọc mẫu..." : "Bấm Play để nghe"}
          </span>
        </div>

        {/* Interactive Text Display */}
        <div className="p-6 rounded-[var(--radius-card)] bg-page border border-[rgba(0,0,0,0.1)] leading-loose text-sm font-medium z-10 relative">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-4">
            {parts.map((part, idx) => {
              if (idx % 2 === 0) {
                // Plain Text
                return (
                  <span key={idx} className="text-text-body font-sans leading-relaxed">
                    {part}
                  </span>
                );
              } else {
                ...
                return (
                  <input
                    ...
                    className={`px-3 py-2 md:py-1 text-base md:text-xs font-bold font-mono text-center rounded-[var(--radius-btn)] bg-card text-text-head placeholder-slate-400 outline-none transition-all border-b-2 w-full md:w-auto ${
                      isCorrect
        ...
        <div className="flex items-center justify-between gap-4 pt-4 border-t border-[rgba(0,0,0,0.1)] z-10 relative">
          
          <button
            onClick={handleReset}
            className="px-4 py-2.5 rounded-[var(--radius-card)] text-xs font-bold bg-card hover:bg-card text-text-body hover:text-slate-900 transition-colors flex items-center gap-1.5 border border-[rgba(0,0,0,0.1)] shadow-sm"
          >
          ...
          {allCorrect ? (
            <div className="px-4 py-2.5 rounded-[var(--radius-card)] bg-teal-500/10 border border-teal-500/20 text-teal-400 text-xs font-extrabold flex items-center gap-1.5">
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-[var(--radius-card)] text-xs font-black bg-primary text-white border-[var(--c-border)] border-primary-dark shadow-[0_4px_0_var(--c-primary-dark)] hover:bg-primary text-white transition-colors flex items-center gap-1.5 shadow-lg shadow-indigo-600/15"
            >
